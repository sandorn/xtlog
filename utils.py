...
def get_function_location(func: Callable[..., object]) -> dict[str, str | int]:
    ...
    try:
        ...
        return {"file": "unknown", "line": 0, "function": getattr(func, "__name__", "unknown")}
...
def simplify_file_path(file_path: str, line_no: int, func_name: str) -> str:
    ...
    try:
        ...
        # 将路径转换为相对路径（相对于项目根目录）
        relative_path: str = os.path.relpath(file_path, project_root) if file_path.startswith(project_root) else file_path
...
def format_record(record: Any) -> None:
    """
    ...
    try:
        # 检查是否有callfrom参数
        callfrom: Callable[..., object] | None = record.extra.get("callfrom")
        if callfrom is not None:
            # 从callfrom函数获取位置信息
            location: dict[str, str | int] = get_function_location(callfrom)
            file_path: str = str(location["file"])
            line_no: int = int(location["line"])
            func_name: str = str(location["function"])
            ...
            # 使用默认的调用位置信息
            default_file_path: str = record.file.path if hasattr(record.file, "path") else str(record.file)
            default_line_no: int = record.line
            default_func_name: str = record.function
            default_simplified_path: str = simplify_file_path(default_file_path, default_line_no, default_func_name)
